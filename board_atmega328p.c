...
#include "board_atmega328p.h"			//we use board-specific defs
...
void mcu_init(void) {
	...
	//stop the timer
	TCCR0B = (TCCR0B &~0x07) | (0x07 & 0x00);		//0x00->stop the timer, 0x03 -> prescaler to 64x:1
	TCNT0 = 0;										//reset the counter
	TIFR0 |= (1<<TOV0);								//1->clear the flag
	TIMSK0 |= (1<<TOIE0);							//1->enable the overflow interrupt, 0->disable the interrupt
	TCCR0B = (TCCR0B &~0x07) | (0x07 & 0x03);		//0x00->stop the timer, 0x03 -> prescaler to 64x:1
